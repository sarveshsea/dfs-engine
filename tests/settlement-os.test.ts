...
const gameLogEntry = (overrides: Partial<PlayerGameLogEntryShape> = {}): PlayerGameLogEntryShape => ({
...
describe('v2 Settlement OS engine', () => {
  ...
  test('keeps league registries isolated per engine instance', async () => {
    ...
    await expect(
      engineWithSim.extractLegStat(simLeg, entry({ legs: [simLeg] }), { actualEntry: gameLogEntry() }),
    ).resolves.toMatchObject({ ok: true, value: 42 });
    await expect(
      defaultEngine.extractLegStat(simLeg, entry({ legs: [simLeg] }), { actualEntry: gameLogEntry() }),
