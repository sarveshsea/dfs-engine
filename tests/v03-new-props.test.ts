...
describe('basketball v0.3 — Double-Double / Triple-Double', () => {
  ...
  test('PTS=15 REB=11 AST=12 → Triple-Double = 1', () => {
    expect(extractStatForPropViaRegistry('Triple-Double', 'NBA', tripleDoubleEntry(), 'prizepicks')).toBe(1);
    expect(extractStatForPropViaRegistry('Double-Double', 'NBA', tripleDoubleEntry(), 'prizepicks')).toBe(1);
  });
  ...
  test('WNBA inherits the basketball table', () => {
    expect(extractStatForPropViaRegistry('Double-Double', 'WNBA', nbaEntry(), 'prizepicks')).toBe(1);
  });
});

describe('NFL v0.3 — Longest Reception / Rush / Pass', () => {
  test('Longest Reception reads receiving.LONG', () => {
    expect(extractStatForPropViaRegistry('Longest Reception', 'NFL', nflWr(), 'prizepicks')).toBe(54);
    expect(extractStatForPropViaRegistry('Longest Reception', 'NFL', nflRb(), 'prizepicks')).toBe(12);
  });
...
describe('MLB v0.3 — Pitching Outs', () => {
  ...
  test('returns null on batter entries (pitcher-only)', () => {
    expect(extractStatForPropViaRegistry('Pitching Outs', 'MLB', mlbBatter(), 'prizepicks')).toBeNull();
  });
});
...
describe('NHL v0.3 — Plus/Minus', () => {
  ...
  test('handles negative values', () => {
    expect(
      extractStatForPropViaRegistry('Plus/Minus', 'NHL', nhlSkater({ plusMinus: '-3' }), 'prizepicks'),
    ).toBe(-3);
  });

  test('null when plusMinus is missing or "-"', () => {
    expect(
      extractStatForPropViaRegistry('Plus/Minus', 'NHL', nhlSkater({ plusMinus: undefined }), 'prizepicks'),
    ).toBeNull();
    expect(
      extractStatForPropViaRegistry('Plus/Minus', 'NHL', nhlSkater({ plusMinus: '-' }), 'prizepicks'),
    ).toBeNull();
  });
