...
// Wider signature matches every other adapter table — every adapter
// receives the source app, even though NHL doesn't branch on it (only
// MLB Hitter FS / Fantasy Score does today). One uniform call shape
// across all leagues so the registry doesn't need typed dispatch.
